...
پروژه Gradle Wrapper کامل و یک اسکریپت یکپارچه برای تست، lint و ساخت APK دیباگ دارد.

1. پوشه را در **Android Studio** باز کنید (نسخه‌ی Ladybug یا جدیدتر).
2. برای بررسی خط فرمان، `./scripts/check.sh` را اجرا کنید.
...
- `UsageSnapshot` — اسنپ‌شات خام دوره‌ای (برای audit و محاسبات delta در فازهای بعد).
  برای جلوگیری از رشد بی‌نهایت دیتابیس، حداکثر یک‌بار در ۱۲ ساعت درج می‌شود و
  ردیف‌های قدیمی‌تر از ۳۰ روز در هر اجرای worker حذف می‌شوند.
